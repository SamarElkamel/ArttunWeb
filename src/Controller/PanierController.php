...
class PanierController extends AbstractController
{
    ...
    public function addToPanier($ref, EntityManagerInterface $entityManager, SessionInterface $session,Security $security): Response
    {// Get the authenticated user
    $user = $security->getUser();

    // Ensure the user is authenticated
    if (!$user) {
        throw $this->createAccessDeniedException('User must be authenticated to add items to the cart.');
    }
    ...
    $client = $userRepository->find($userId);
    
    // Get the product based on the reference
    ...
    // If the product is already in the cart, redirect with an error message
    if ($existingPanier) {
        $session->getFlashBag()->add('error', 'This product is already added to the cart.');
    ...
    $panier->setIdClient($client); // Set the client as the User entity
    ...
    $session->getFlashBag()->add('success', 'Product added to the cart successfully.');
    ...
    public function checkout(Request $request, EntityManagerInterface $entityManager, $stripeSK,Security $security): Response
    {
